...
//error_reporting(E_ALL & ~E_NOTICE);
	...
	$serverpath = 'http://'.$_SERVER['HTTP_HOST'].'/android/audios/'; //only for hybrid testing demos link
	define("DB_NAME",$dbs);
...
$qry_st  = "SELECT * FROM ".TBL_CONTACT." WHERE c_id='1'" ;
	if($db->query($qry_st) && $db->get_num_rows() > 0){
	   ...
	   }date_default_timezone_set($status[0]['time_zone']);
	define("GEOCODE",$status[0]['google_coordinates']);
	      $st = $status[0]['st'];	
		  $_SESSION['st'] = $st;
