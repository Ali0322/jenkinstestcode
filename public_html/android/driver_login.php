...
include_once('DatabaseUS.inc.php');
	...
	{	$jsonarray['status'] = 'true'; $data = $db->fetch_all_assoc(); $jsonarray['data'] = $data;
	/* $ct=sizeof($data);
	$jsonarray['data'][$ct]['id'] = '000';
	$jsonarray['data'][$ct]['vnumber'] = 'NOSELECTION';
	$jsonarray['data'][$ct]['vname'] = 'No Selection';*/
...
if (isset($_GET['username']) && isset($_GET['password'])) {
	...
	$jsonarray = array();
		  ...
		  $jsonarray['error'] = 'Login Failed';
			  ...
			  $querylogin = "UPDATE drivers SET login_status = '1',udid='".sql_replace($_GET['udid'])."' WHERE Drvid = '".sql_replace($data[0]['Drvid'])."' ";
			...
			$queryinsertdvmap = "INSERT INTO dv_mapping SET 
				...
				if($id != '000'){  $db->execute($queryinsertdvmap); }
				    }*/
	...
	if(isset($_GET['action']) && $_GET['action'] == 'logout'){
		if (isset($_GET['driverID'])) {
			$querylogout = "UPDATE drivers SET login_status = '0',udid='' WHERE   drv_code = '".sql_replace($_GET['driverID'])."' ";
			//$querylogout = "UPDATE drivers SET login_status = '0',udid='' WHERE   Drvid = '".sql_replace($_GET['driverID'])."' ";
			  if($db->execute($querylogout)){
