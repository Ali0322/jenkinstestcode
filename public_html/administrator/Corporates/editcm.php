...
//GET HOSPITALS LIST
  ...
  //UPDATE CASE MANAGER
	  ...
	  $ustatus   = sql_replace($_POST['ustatus']);	
       ...
       if(!isset($hospid) || $hospid == ''){
		   $error .= "<img src='images/bulit.gif'> Clinic not selected!</img><br>";
		   }
  ...
  if(!$error) {
		 ...
		 $Query  = "UPDATE ".TBL_CM." SET 
		  ...
		  if($db->execute($Query))
		    { 
			  ...
			  echo '<script>alert("Casemanager Updated Successfully.");</script>';
                          echo '<script>location.href="casemanagers.php";</script>';
                          exit;
			}else{
    ...
    $pgTitle = "Admin Panel -- Corporation [Edit - Case Manager]";
