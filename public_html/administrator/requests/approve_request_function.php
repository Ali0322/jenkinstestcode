	...
	function get_veh($drv)
			...
			{ $google = new mapquest_google_miles;	
				...
				if($db->query($req_query) && $db->get_num_rows() > 0)
				{
					...
					$query = "SELECT * FROM ".TBL_SHEET." WHERE dated = '".$req_info[0]['today_date']."'";
...
insert_tdetail($id,$trip_id,$apptime,$pickaddr,$destination,'AB',$miles_string[0],$reqsid,$c1,$c2,$req_info[0]['picklocation'],$req_info[0]['droplocation'],$req_info[0]['p_phnum'],$req_info[0]['d_phnum'],$req_info[0]['pickup_instruction'],$req_info[0]['destination_instruction'],$req_info[0]['legaid']);  		}
	if($req_info[0]['triptype'] == 'Round Trip'){
		
insert_tdetail($id,$trip_id,$apptime,$pickaddr,$destination,'AB',$miles_string[0],$reqsid,$c1,$c2,$req_info[0]['picklocation'],$req_info[0]['droplocation'],$req_info[0]['p_phnum'],$req_info[0]['d_phnum'],$req_info[0]['pickup_instruction'],$req_info[0]['destination_instruction'],$req_info[0]['legaid']);
insert_tdetail($id,$trip_id,$returnpickup,$destination,$backto,'BF',$miles_string[1],$reqsid,$c2,$c6,$req_info[0]['droplocation'],$req_info[0]['backtolocation'],$req_info[0]['d_phnum'],$req_info[0]['p_phnum'],$req_info[0]['destination_instruction'],$req_info[0]['backto_instruction'],$req_info[0]['legbid']);   		}
	
	if($req_info[0]['triptype'] == 'Three Way') 	{
		
insert_tdetail($id,$trip_id,$apptime,$pickaddr,$destination,'AB',$miles_string[0],$reqsid,$c1,$c2,$req_info[0]['picklocation'],$req_info[0]['droplocation'],$req_info[0]['p_phnum'],$req_info[0]['d_phnum'],$req_info[0]['pickup_instruction'],$req_info[0]['destination_instruction'],$req_info[0]['legaid']);
insert_tdetail($id,$trip_id,$three_pickup,$destination,$three_address,'BC',$miles_string[1],$reqsid,$c2,$c3,$req_info[0]['droplocation'],$req_info[0]['droplocation2'],$req_info[0]['d_phnum'],$req_info[0]['d_phnum2'],$req_info[0]['destination_instruction'],$req_info[0]['destination_instruction2'],$req_info[0]['legbid']);
insert_tdetail($id,$trip_id,$returnpickup,$three_address,$backto,'CF',$miles_string[2],$reqsid,$c3,$c4,$req_info[0]['droplocation2'],$req_info[0]['backtolocation'],$req_info[0]['d_phnum2'],$req_info[0]['p_phnum'],$req_info[0]['destination_instruction2'],$req_info[0]['backto_instruction'],$req_info[0]['legcid']);  }
	if($req_info[0]['triptype'] == 'Four Way'){
		
insert_tdetail($id,$trip_id,$apptime,$pickaddr,$destination,'AB',$miles_string[0],$reqsid,$c1,$c2,$req_info[0]['picklocation'],$req_info[0]['droplocation'],$req_info[0]['p_phnum'],$req_info[0]['d_phnum'],$req_info[0]['pickup_instruction'],$req_info[0]['destination_instruction'],$req_info[0]['legaid']);
insert_tdetail($id,$trip_id,$three_pickup,$destination,$three_address,'BC',$miles_string[1],$reqsid,$c2,$c3,$req_info[0]['droplocation'],$req_info[0]['droplocation2'],$req_info[0]['d_phnum'],$req_info[0]['d_phnum2'],$req_info[0]['destination_instruction'],$req_info[0]['destination_instruction2'],$req_info[0]['legbid']);
insert_tdetail($id,$trip_id,$four_pickup,$three_address,$four_address,'CD',$miles_string[2],$reqsid,$c3,$c4,$req_info[0]['droplocation2'],$req_info[0]['droplocation3'],$req_info[0]['d_phnum2'],$req_info[0]['d_phnum3'],$req_info[0]['destination_instruction2'],$req_info[0]['destination_instruction3'],$req_info[0]['legcid']);
insert_tdetail($id,$trip_id,$returnpickup,$four_address,$backto,'DF',$miles_string[3],$reqsid,$c4,$c5,$req_info[0]['droplocation3'],$req_info[0]['backtolocation'],$req_info[0]['d_phnum3'],$req_info[0]['p_phnum'],$req_info[0]['destination_instruction3'],$req_info[0]['backto_instruction'],$req_info[0]['legdid']); }
...
insert_tdetail($id,$trip_id,$returnpickup,$five_address,$backto,'EF',$miles_string[4],$reqsid,$c5,$c6);  }
	...
	function insert_tdetail($id,$trip_id,$picktime,$pickaddress,$dropaddress,$tripdirection,$trp_miles,$reqsid,$cp,$cd,$picklocation,$droplocation,$p_phnum,$d_phnum,$pickup_instruction,$destination_instruction,$ccode)
			{	$db = new Database;	
				$db->connect();
				//$trip_id = mysql_insert_id();
				$req_query = "SELECT * FROM ".TBL_FORMS." WHERE id = '".$id."' ";
				...
				$tQuery = " INSERT INTO ".TBL_TRIP_DET." SET 
									...
									clinic	 		= 		'$clinic',
									phyaddress 		= 		'".sql_replace($phyaddress)."',
									phyphone 		= 		'$phyphone',
									phyfax	 		= 		'$phyfax',
									reason 			= 		'$reason',
									reqid 			= 		'$reqsid',
