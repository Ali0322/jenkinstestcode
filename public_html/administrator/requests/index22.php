...
if(($_POST['monday']=='on')&&(!empty($_POST['tdmonday']))&&(!empty($_POST['aptmonday']))) {
 ...
 //INSERTING REOCCURING TRIPS
				rectrips($day,$till_date,$apptimeR,$returnpickupR,$account, $unloadedmiles,$miles_string,$miles,$totcharges,$triptype,$vehtype,$ssn,$req_id,$pickadd,$destination_one,$destination_two,$destination_three,$destination_four,$three_pickup,$four_pickup,$five_pickup,$destination_last,$appdate,$casemanager1,$pname,$dob,$casemanager2,$cisid,$driver,$sex,$childseat,$escort,$wchair,$stretcher,$dstretcher,$oxygen,$comments,$status,$apptype,$insurance_name,$org_apptime,$phnum,$pickup_instruction,$billing_address,$po,$patient_weight,$bar_stretcher,$c1,$c2,$c3,$c4,$c5,$c6,$capped_miles,$after_hours,$dwchair,$b_accountname,$destination_instruction,$backto_instruction,$picklocation,$droplocation,$backtolocation,$d_phnum);
			 } //End for Monday
			 //Start For tuseday
...
if(($_POST['sunday']=='on')&&(!empty($_POST['tdsunday']))&&(!empty($_POST['aptsunday']))) {
 ...
 //INSERTING REOCCURING TRIPS
	...
	function numWeekdays( $start_ts, $end_ts, $day, $include_start_end = false) {  //Count the number of days with dates.    
	   ...
	   function rectrips($day,$till_date,$apptime,$returnpickup,$account,$unloadedmiles,$miles_string,$miles,$totcharges,$triptype,$vehtype,$ssn,$req_id,$pickadd,$destination_one,$destination_two,$destination_three,$destination_four,$three_pickup,$four_pickup,$five_pickup,$destination_last,$appdate,$casemanager1,$pname,$dob,$casemanager2,$cisid,$driver,$sex,$childseat,$escort,$wchair,$stretcher,$dstretcher,$oxygen,$comments,$status,$apptype,$insurance_name,$org_apptime,$phnum,$pickup_instruction,$billing_address,$po,$patient_weight,$bar_stretcher,$c1,$c2,$c3,$c4,$c5,$c6,$capped_miles,$after_hours,$dwchair,$b_accountname,$destination_instruction,$backto_instruction,$picklocation,$droplocation,$backtolocation,$d_phnum){ //START of Reoccur Trips
	   $dbus = new Database;	
	    $dbus->connect();
		$q = "SELECT * FROM ".TBL_CONTACT;
