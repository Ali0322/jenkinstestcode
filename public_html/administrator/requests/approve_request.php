...
$loadedmilage 	= ($totmilages); 
	...
	{
			  $Q=" SELECT h.rate_type,h.id FROM ".TBL_HOSPITALS." as h, requests as r WHERE r.userid = h.id AND r.reqid = '$rqid'";
	...
	$totcharges = round((($base 	  *	$r['pickup_ch']) + 
		...
		//-------------------------------------- Start of Functions------------------------
			...
			if($db->query($req_query) && $db->get_num_rows() > 0)
			{
				$req_info = $db->fetch_all_assoc();
				 insert_trip($id);
		 $hospQuery = "SELECT h.id,h.email_address,h.hospname,r.userid,r.reqid,f.* 		          
		  FROM ".TBL_FORMS." as f, ".TBL_HOSPITALS." as h, ".TBL_REQUESTS." as r 
		  WHERE f.id='".$rid."' AND h.id=r.userid AND r.reqid=f.req_id GROUP BY f.id";
		...
		if you have any questions, contact us at <a href="mailto:'.$emaildata[0]['email'].'">'.$emaildata[0]['email'].'</a>
            ...
            $to=$driverdata[0]['cell_num'];
  		    $req= "SELECT * FROM ".TBL_FORMS." as r, ".TBL_HOSPITALS." as rs WHERE r.id = '".$rid."' ";
