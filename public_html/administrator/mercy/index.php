...
$hospitalsq = "SELECT * FROM ".TBL_HOSPITALS." ORDER BY hospname ASC "; 
	if($db->query($hospitalsq) && $db->get_num_rows() > 0){     $hospitals= $db->fetch_all_assoc();  	}
//GET STATES LIST
...
$gappt="SELECT * FROM ".appointment_type; if($db->query($gappt) && $db->get_num_rows() >0){$appdata = $db->fetch_all_assoc();}		 
	...
	if($hostpital_id==''){$hostpital_id=$_POST['hostpital_id_drop'];}
	if($hostpital_id==''){}
	
	$hostpital_id='16';
	
	$Queryp = "SELECT prog_type,hospname,rate_type FROM hospitals WHERE id='$hostpital_id' "; 
	if($db->query($Queryp) && $db->get_num_rows() > 0)  	{ $Detailsp = $db->fetch_all_assoc(); }	         
	  $progtype     = $Detailsp[0]['prog_type'];
	  $hosname      = $Detailsp[0]['hospname'];
...
if(($_POST['sunday']=='on')&&(!empty($_POST['tdsunday']))&&(!empty($_POST['aptsunday']))) {	 $rec_count=1;
			...
			$till_date 		= sql_replace($_POST['tdsunday']); 
			$apptimeR 		= sql_replace($_POST['aptsunday']); 
			$returnpickupR 	= sql_replace($_POST['retsunday']); //print_r($_POST); exit;
 //INSERTING REOCCURING TRIPS
				rectrips($day,$till_date,$apptimeR,$returnpickupR,$account,$unloadedmiles,$miles_string,$miles,$totcharges,$triptype,$vehtype,$ssn,$req_id,$pickadd,$destination_one,$destination_two,$destination_three,$destination_four,$three_pickup,$four_pickup,$five_pickup,$destination_last,$appdate,$casemanager1,$pname,$dob,$casemanager2,$cisid,$driver,$sex,$childseat,$escort,$wchair,$stretcher,$dstretcher,$oxygen,$comments,$status,$apptype,$insurance_name,$org_apptime,$phnum,$pickup_instruction,$billing_address,$po,$patient_weight,$bar_stretcher,$c1,$c2,$c3,$c4,$c5,$c6,$capped_miles,$after_hours,$p_after_hours,$r_after_hours,$dwchair,$b_accountname,$destination_instruction,$backto_instruction,$picklocation,$droplocation,$backtolocation,$d_phnum,$SQ);
			 } //End for sunday			 
 ...
 /*$from = 'requests@xcv.com';
	...
	function numWeekdays( $start_ts, $end_ts, $day, $include_start_end = false) {  //Count the number of days with dates.    
	   ...
	   function rectrips($day,$till_date,$apptime,$returnpickup,$account,$unloadedmiles,$miles_string,$miles,$totcharges,$triptype,$vehtype,$ssn,$req_id,$pickadd,$destination_one,$destination_two,$destination_three,$destination_four,$three_pickup,$four_pickup,$five_pickup,$destination_last,$appdate,$casemanager1,$pname,$dob,$casemanager2,$cisid,$driver,$sex,$childseat,$escort,$wchair,$stretcher,$dstretcher,$oxygen,$comments,$status,$apptype,$insurance_name,$org_apptime,$phnum,$pickup_instruction,$billing_address,$po,$patient_weight,$bar_stretcher,$c1,$c2,$c3,$c4,$c5,$c6,$capped_miles,$after_hours,$p_after_hours,$r_after_hours,$dwchair,$b_accountname,$destination_instruction,$backto_instruction,$picklocation,$droplocation,$backtolocation,$d_phnum,$SQ){ //START of Reoccur Trips
