...
function pickuptime($number){
	...
	if($total_seconds > 60*60){$total_seconds =($total_seconds-(30*60));}
...
function gen_data($data,$db,$dated){ 
			...
			if (strpos($data[12], 'X') !== false) {}else{
			...
			$d_phnum					= phone_format(str_replace("-","",trim(str_replace("'"," ",($data[32])))));
			  $comments 	  			= trim(str_replace("'"," ",$data[33])).' - '.trim(str_replace("'"," ",$data[34]));   
		 	  $today_date 				=  date('Y-m-d');//dateted_format(trim($data[38])); 
	//   echo exit;  
			   $vehicle_abbr 	  			= (trim(str_replace("'"," ",$data[13]))); 
...
$Q33="SELECT * FROM ".TBL_FORMS." WHERE appdate = '".$appdate."' AND legaid='$legid' AND reqstatus !='disapproved' || 
	...
	echo $Q4="SELECT * FROM ".TBL_FORMS." WHERE appdate = '".$appdate."' AND clientname = '".$pname."' ORDER BY id DESC LIMIT 1 "; exit;
	...
	if($org_apptime=='00:00' || $org_apptime=='00:00:00'){$org_apptime='Will Call';}
	...
	$db->execute($Q5);
	 ...
	 $Qaccounts  = "SELECT id FROM " .  accounts ." WHERE TRIM(LOWER(account_name))='".strtolower(trim('MTM'))."' " ;
	...
	$getWorksheetName = array();
		...
		echo '<script>alert("Trip Sheet Uploaded Successfully");</script>'; 
	      ...
	      echo '<script>javascript:history.back();</script>'; exit;
			...
			$smarty->display('mercytpl/add_listing_csv.tpl'); 
/*
Array ( 
[0] => 8138692715 
	[1] => CHAITLALL 
