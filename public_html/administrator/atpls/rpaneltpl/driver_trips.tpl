<html>
<head>
{literal}<script> var drv_id ='{/literal}{$dri_code}{literal}';</script>{/literal}
...
function initialize() {
	$.post("../google_api_log.php", {type: ""+'Maps Javascript API'}, function(data2){ });
  geocoder = new google.maps.Geocoder();
 
  var haightAshbury = new google.maps.LatLng('{/literal}{$smarty.const.GEOCODE}{literal}');//42.0058862,-88.1714097
  var mapOptions = {
    zoom: 10,
...
<div style=" width:240px; height:70px; float:left;"><a href="http://hybridtracktrans.com" target="_blank"><img src="../images/logo.png" height="70" width="220" ></a>
...
setInterval("loopmaker();", 3000);
    //calcRoute(pickadd,dropadd);
	
	
</script>	{/literal}
{section name=q loop=$address}
{$smarty.section.q.iteration}
{literal}<script> var pickadd ='{/literal}{$a}{literal}';</script>{/literal}
{literal}<script> var dropadd ='{/literal}{$b}{literal}';</script>{/literal}
{literal}<script>
//calcRoute(pickadd,dropadd);
