          ...
          <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td class="tabs"><ul>
                    ...
                    <li {if $st== '4'} class="active"{/if}><a href="grid2.php?id={$id}&st=4&ad=0">Delivered ({$st4})</a></li>
                    <li {if $st== '3'} class="active"{/if}><a href="grid2.php?id={$id}&st=3&ad=0">Cancelled ({$st3})</a></li>
...
<a href="#" onclick="findcoord('drop','{$membdetail[q].drp_add}','{$membdetail[q].tdid}');" >{if $membdetail[q].drop_latlong eq '' || $membdetail[q].drop_latlong eq 'NULL'}<img src="../images/icons/null_cord.png" title="Find Coordinate" >{else}<img src="../images/icons/yes_cord.png" title="Find Updated Coordinate" >{/if}</a>--></td>
                      ...
                      <td align="left" valign="top" class="grid_icon"><!--<a  href="#" onclick="popWind('editgrid-new.php?id={$membdetail[q].tdid}&type={if $st eq 6}1{else}2{/if}');" title="Edit"> <img border="0" alt="Edit" src="../graphics/edit.png"></a>--><a  href="edit2.php?id={$membdetail[q].reqid}" title="Edit" target="_blank">  <img border="0" alt="Edit" src="../graphics/edit.png"></a>&nbsp;&nbsp;		  
    {if $st eq 4}  <a href="javascript:popWind('../reports/details.php?id={$membdetail[q].tdid}');" title="View"> {if $membdetail[q].status eq '4'}
                     ...
                     {if $smarty.session.admuser.admin_level eq '0'} <a href="#"  onclick="return deleteRec('{$membdetail[q].tdid}','{$id}');"  title="Remove"> <img alt="Remove" border="0"  src="../graphics/delete.png"></a> {/if}&nbsp; 
                        ...
                        <select name="st_status" id="st_status" onchange="st_status_change('{$membdetail[q].tdid}',this.value);" style="width:120px;" >
 {if $membdetail[q].status eq '5' ||  $membdetail[q].status eq '2' || $membdetail[q].status eq '0'}                       
                          <option value="5" {if $membdetail[q].status eq '5'} selected="selected"{/if}>In Progress</option>
                          <option value="2" {if $membdetail[q].status eq '2'} selected="selected"{/if}>Rescheduled</option>
                          ...
                          <option value="8" {if $membdetail[q].status eq '8'} selected="selected"{/if}>Not Going</option>
                          <option value="7" {if $membdetail[q].status eq '7'} selected="selected"{/if}>Not at home</option>
{elseif $membdetail[q].status eq '3'}	
                          <option value="5" {if $membdetail[q].status eq '5'} selected="selected"{/if}>In Progress</option>
                          <option value="3" {if $membdetail[q].status eq '3'} selected="selected"{/if}>Cancelled</option>
{elseif $membdetail[q].status eq '6'}
                          <option value="5" {if $membdetail[q].status eq '5'} selected="selected"{/if}>Picked Up</option>
