        ...
        <tr>
          <td align="center"  valign="top"><table width="100%" border="0" class="main_table">
              ...
              <tr bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
                ...
                <td align="center" valign="middle"><select name="vtype" id="vtype" class="SelectBox required" onchange="return dvmap(this.value,'{$drvdetails[q].Drvid}')" >
                    <option value="">Select</option>
                    {section name=n loop=$vlist}
			          
                    <option value="{$vlist[n].id}^{$vlist[n].vnumber}" {if $vlist[n].id eq $vtype}selected{/if}>{$vlist[n].vnumber} -  {$vlist[n].vname}</option>
                    {/section}
