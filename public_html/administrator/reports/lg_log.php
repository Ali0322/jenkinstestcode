<?php
	...
	{ 
		...
		$logiticid 		=  sql_replace($_GET['logiticid']);
		...
		if($stdate!= '' && $enddate!='')
		{
			...
			$date2=date_create($enddate);
				 $diff=date_diff($date1,$date2);
						$rang = $diff->format("%a");
			if($rang>$LIMIT_DAYS){
			//$enddate = date("Y-m-d",strtotime($stdate." +".$ADD_DAYS." day"));  
			$msgs='<span style="color:Red"; ></span>';}
			// Data Range ended
     ...
     $query = "SELECT td.*,t.trip_clinic,tr.invoice_gen, CONCAT(d.fname,' ',d.lname) as name,t.trip_user, d.drv_code,t.account 
			FROM ".TBL_TRIP_DET." td 
			LEFT JOIN ".TBL_TRIPS." AS t ON ( t.trip_id = td.trip_id ) 
			left join ".TBL_DRIVERS." AS d on (d.drv_code = td.drv_id)
			left join ".TBL_FORMS." as tr on td.reqid=tr.id
		...
		{	$totalRows = $db->get_num_rows();
		   ...
		   {	if($data[$i]['invoice_gen']==0){ invoice_generation($data[$i]['reqid']); }
	...
	if($db->query($query) && $db->get_num_rows() > 0)
	{	$driver = $db->fetch_all_assoc();	} //WHERE TRIM(LOWER(account_name))='".strtolower(trim('Logistic Care'))."'
  $Queryhosp1 = "SELECT ac.id,ac.account_name FROM ".accounts." ac  WHERE TRIM(LOWER(REPLACE(account_name,' ','')))='".strtolower(trim('Logistic Care'))."' ORDER BY ac.account_name ASC";
   if($db->query($Queryhosp1) && $db->get_num_rows() > 0)
	...
	$smarty->display('reportstpl/lg_log.tpl');
?>