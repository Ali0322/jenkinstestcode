<?php
	...
	{ 
	   ...
	   elseif($status == ''){ $whr .= "AND td.status IN ('1','4','6','3','8','7','9','10') ";   }
		...
		//if($hospname!= '')
		{$whr .= " AND t.account = '".$_SESSION['logiticid']."'";}
   	 // $pagination = new pagination($_GET['pageNum'],$maxRows=10,$totalRows); 
    $query = "SELECT td.*,bl.charges,t.trip_clinic,d.fname as dfname,d.lname as dlname, CONCAT(d.fname,' ',d.lname) as name,d.license,t.trip_user, d.drv_code,d.Drvid,vt.vcode,d.signature as drsignature,vt.vehtype
			...
			LEFT OUTER JOIN ".billing_info." AS bl ON ( bl.tdid = td.tdid ) 
	...
	$smarty->display('reportstpl/lg_log_print.tpl');
?>