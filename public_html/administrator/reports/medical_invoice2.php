...
$whr_clz .= "  AND tr.appdate BETWEEN '".convertDateToMySQL($startdate)." 00:00:00' AND '".convertDateToMySQL($enddate)." 23:59:59'";	  		  }
		     if($hospname != '' && $hospname != '0'){
              $whr_clz .= "  AND tr.account = '".$hospname."'";   		  
		  }		         
	         if($pname != ''){
            $whr_clz .= "  AND LTRIM(LOWER(tr.clientname)) = '".strtolower(trim($pname))."'";   		  
		  }	

 $query2 = "SELECT * FROM contact_info WHERE c_id = '1' ";
		if($db->query($query2) && $db->get_num_rows()>0){$cdata = $db->fetch_one_assoc(); }
	   $query="SELECT ac.id,ac.freemiles as frmiles,ac.account_name,ac.address,ac.city,ac.state,ac.zip,tr.* FROM request_info as tr LEFT JOIN ".accounts." as ac on tr.account=ac.id WHERE 1 $whr_clz  LIMIT 1";
	   
		if($db->query($query) && $db->get_num_rows() > 0)
		{ 
		$tdata = $db->fetch_one_assoc(); //print_r($tdata);
		 $queryB = "SELECT bl.*,tr.three_address,tr.four_address,tr.five_address,tr.triptype,tr.vehtype,tr.pickaddr,tr.destination,tr.backto,tr.clientname,tr.appdate,tr.apptime,tr.returnpickup,tr.three_pickup,tr.four_pickup,tr.phnum,tr.id as tid,tr.po,tr.legaid,tr.legbid,tr.legcid,tr.legdid,tr.dob FROM billing_info as bl left join ".TBL_FORMS." as tr on bl.trip_id=tr.id WHERE 1  AND tr.reqstatus = 'approved'  AND bl.cancel = '0' $whr_clz ORDER BY bl.id";
	...
	$smarty->display('reportstpl/medical_invoice2.tpl');
?>