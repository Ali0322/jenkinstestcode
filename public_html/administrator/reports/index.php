...
$whr_clz .= "  AND $by_date BETWEEN '".convertDateToMySQL($startdate)." 00:00:00' AND '".convertDateToMySQL($enddate)." 23:59:59'";	  		  }
		     if($hospname != '' && $hospname != '0'){
              $whr_clz .= "  AND r.account = '".$hospname."'";   		  
		  }		  
	       if($cisid != ''){
 ...
 if($glueids != ''){	 
  ...
  /* if($hospname == '' && $cisid != ''){
	...
	$showClient = 'no';	
     ...
     $Querypg = "SELECT COUNT(*) FROM ".TBL_FORMS." r $whr_clz ";	
...
if($totalRows > 0){
$Query = "SELECT r.id, ac.account_name, ac.account_name, r.account, r.cisid, ".$qryquote."
(SELECT COUNT(id) FROM ".TBL_FORMS." r ".$whr_clz." and r.reqstatus='active' AND r.account=ac.id ) active,
(SELECT COUNT(id) FROM ".TBL_FORMS." r  ".$whr_clz." and r.reqstatus='approved' AND r.account=ac.id) approved,
(SELECT COUNT(id) FROM ".TBL_FORMS." r ".$whr_clz." and r.reqstatus='disapproved' AND r.account=ac.id) disapproved,
(SELECT COUNT(id) FROM ".TBL_FORMS." r ".$whr_clz." and r.reqstatus IN('active', 'approved', 'disapproved') AND r.account=ac.id) pending
FROM ".TBL_FORMS." r
LEFT JOIN accounts AS ac ON r.account=ac.id ".$whr_clz." and r.account!=0
GROUP BY r.account
LIMIT ".$pagination->startRow . ",".$pagination->maxRows;	  
	 if($db->query($Query) && $db->get_num_rows() > 0)
		  { $Requests = $db->fetch_all_assoc(); }
     $pages =  $pagination->display_pagination();	
	 $Query77 = "SELECT * FROM ".TBL_FORMS." r $whr_clz ";
	 ...
	 //print_r($Requests77);        	
//DATA
	$g2data1 = "SELECT COUNT(*) FROM ".TBL_FORMS." r  $whr_clz AND reqstatus='active'";
    $g2activeReqs = $db2->executeScalar($g2data1);
	$g2data2 = "SELECT COUNT(*) AS rows FROM ".TBL_FORMS." r $whr_clz AND reqstatus='approved'";
    $g2appReqs = $db2->executeScalar($g2data2);
	$g2data3 = "SELECT COUNT(*) AS rows FROM ".TBL_FORMS." r $whr_clz AND reqstatus='disapproved'";
