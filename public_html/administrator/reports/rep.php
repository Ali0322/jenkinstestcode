<?php
	...
	$qcount  = "SELECT  td.trip_miles, td.status 
     ...
     $query = "SELECT td.tdid, td.date, td.phyaddress, t.trip_clinic, CONCAT(d.fname,' ',d.lname) as name,t.trip_user, d.drv_code, td.pck_add, td.drp_add, td.pck_time,td.drv_id,td.escort_id,td.drp_time, td.aptime,td.ac_noshowcancell,
			td.drp_atime, td.trip_miles, td.pickStatus, td.status 
			FROM ".TBL_TRIP_DET." td 
	...
	$qcount  = "SELECT  td.trip_miles, td.status 
   	...
   	// $pagination = new pagination($_GET['pageNum'],$maxRows=40,$totalRows); 
     $query = "SELECT td.tdid, td.date,td.type, td.phyaddress, t.trip_clinic, CONCAT(d.fname,' ',d.lname) as name,t.trip_user, d.drv_code, td.pck_add, td.drp_add, td.pck_time,td.drv_id,td.escort_id,td.drp_time, td.aptime,td.ac_noshowcancell,
			td.drp_atime, td.trip_miles, td.pickStatus, td.status 
			FROM ".TBL_TRIP_DET." td 
			left outer join ".TBL_DRIVERS." AS d on (d.drv_code = td.drv_id)
			LEFT OUTER JOIN ".TBL_TRIPS." AS t ON ( t.trip_id = td.trip_id ) 
		...
		if($db->query($query) && $db->get_num_rows() > 0)
		{
			...
			//print_r($tdata); exit;
		//CSV writing
		$header="Patient Name,Leg,Driver,Pick Up Address,Drop Address,Pick Time,Actual Pick Time,Total Miles,Status \n";
	...
	$array = array("~", "\n","\n\r","<br/>","<br>",",","\r");	
			...
			$csv_str = trim($tdata[$i]['trip_user']).','.$leg.','.str_replace($array,'-',$tdata[$i]['name']).','.str_replace($array,'-',$tdata[$i]['pck_add']).','.str_replace($array,'-',$tdata[$i]['drp_add']).','.$tdata[$i]['pck_time'].','.$tdata[$i]['aptime'].','.$tdata[$i]['trip_miles'].','.$status;  
