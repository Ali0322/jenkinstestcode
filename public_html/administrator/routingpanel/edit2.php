...
if(isset($_POST['submit']) && $_POST['submit'] == 'Update'){
  ...
  $Query2  = "UPDATE ".TBL_FORMS." SET
		  ...
		  if($db->execute($Query2))
			...
			$Quptripsdetail2="INSERT INTO trip_details SET $Qrst, reqid ='".$id."', type = 'BF',trip_id = '".$trip_id."' "; }else{
				...
				{	
					...
					$wc 		= '1';	
											...
											$Qrst="	date					=	'".$appdate."',
													...
													passenger 				= 	'".$passenger."'";	
			if($pre_triptype=='One Way'){
			$Quptripsdetail2="INSERT INTO trip_details SET $Qrst, reqid ='".$id."', type = 'BC',trip_id = '".$trip_id."' ";	}else{												
			...
			$Quptripsdetail2="INSERT INTO trip_details SET $Qrst, reqid ='".$id."', type = 'CF',trip_id = '".$trip_id."' ";	}else{												
			...
			$Quptripsdetail2="UPDATE trip_details SET date					=	'".$appdate."',
													veh_id					=	'".$vehtype."',
													pck_add					=	'".$destination_one."',
													drp_add					=	'".$destination2."',
													picklocation			=	'".$droplocation."',
			...
			if($pre_triptype=='Round Trip' && $triptype=='One Way'){
				$db->execute("DELETE FROM trip_details WHERE trip_id ='".$trip_id."' AND type IN ('BF','BC','CF','CD','DF')");}
			if($pre_triptype=='Three Way' && $triptype=='One Way'){
				$db->execute("DELETE FROM trip_details WHERE trip_id ='".$trip_id."' AND type IN ('BF','BC','CF','CD','DF')");}	
			if($pre_triptype=='Three Way' && $triptype=='Round Trip'){
