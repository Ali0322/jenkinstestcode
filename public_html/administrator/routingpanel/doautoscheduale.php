...
function get_trip($db,$today,$limit,$veh_type_id=''){  sleep(2);  $qadd="";
  ...
  $Qearynexttrip="SELECT tdid,pck_time,drp_time,pick_latlong,drop_latlong,pck_add,drp_add,passenger,veh_id FROM trip_details WHERE date = '$today' AND drv_id = '' AND wc = '0' AND status IN ('9','5') $qadd ORDER BY pck_time ASC, tdid ASC LIMIT $limit "; 
...
$refinedriver = multi_sort($reachable_driver, "distance", false);
		if($refinedriver){
			...
			if($assignkar==1){
				...
				$QassigndriverX= "UPDATE trip_details SET drv_id = '".$drv_code_USS."'  WHERE tdid = '".$moretrips[$x]['tdid']."' ";
