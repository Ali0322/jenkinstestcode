...
$Query = "SELECT f.* FROM ".TBL_FORMS." as f WHERE LTRIM(LOWER(clientname))='".strtolower(trim($pname))."' ORDER BY id DESC LIMIT 1";
		if($db->query($Query) && $db->get_num_rows() > 0) { $Details = $db->fetch_all_assoc(); }
	 	$Query2 = "SELECT * FROM ".patient." WHERE TRIM(LOWER(REPLACE(name,' ','')))='".strtolower(trim(str_replace(' ','',$pname)))."' ORDER BY id DESC LIMIT 1";
				if($db->query($Query2) && $db->get_num_rows() > 0) { $Details2 = $Details3 = $db->fetch_all_assoc(); }   }
	$Details2 = $Details;
			//print_r($Details3);
			//exit;	//$phyaddr = explode(",",$Details[0]['phyaddress']);
				//$ind = count($phyaddr);
				...
				$pickaddress = trim($pickaddr[0]);
				$pickroom = trim($pickaddr[1]);*/
				
				$pickaddr = explode(",",$Details2[0]['pickaddr'],3);
				//$pickzip = trim($pickaddr[4]);
				//$pickstate = trim($pickaddr[3]);
				//$pickcity = trim($pickaddr[2]);
				$pickaddress = trim($pickaddr[0]).','.trim($pickaddr[2]);
				$pickroom = trim($pickaddr[1]);
				...
				$returnedData .= trim($Details3[0]['name']).'^'.$Details3[0]['phone'].'^'.$Details[0]['email'];
				$returnedData .= '^'.$Details3[0]['dob'].'^'.$Details2[0]['po'];	
...
/*$returnedData .= '^'.$Details2[0]['address'].'^'.$Details2[0]['city'].'^'.$Details2[0]['state'].'^'.$Details2[0]['zip'];*/			
				...
				$returnedData .= '^'.$Details[0]['triptype'].'^'.$Details[0]['vehtype'].'^'.$Details[0]['org_apptime'];//37
				$returnedData .= '^'.$Details[0]['apptime'].'^'.$Details2[0]['sex'].'^'.$Details[0]['org_apptime'];//40					
